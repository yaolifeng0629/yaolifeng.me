            ...
            process.use(() => (tree) => {
                visit(tree, 'element', (node) => {
                    if (node.tagName === 'a') {
                        ...
                        // 添加基础类名
                        node.properties.className = 'bytemd-pretty-link';

                        // 创建链接容器
                        const linkContainer = {
                            type: 'element',
                            tagName: 'span',
                            properties: { className: 'link-container' },
                            children: [],
                        };
                        // 判断当前链接类型并添加对应图标
                        ...
                        if (svgNode) {
                            linkContainer.children.push(svgNode);
                        }
                        // 添加原始内容
                        linkContainer.children.push(...node.children);

                        // 添加外部链接图标
                        linkContainer.children.push(linkSvgNode);
                        // 创建悬浮链接提示
                        const hoverLink = {
                            type: 'element',
                            tagName: 'span',
                            properties: { className: 'hover-link' },
                            children: [{ type: 'text', value: node.properties.href }],
                        };
                        // 更新节点的children
                        node.children = [linkContainer, hoverLink];
